---
title: Ghost Pages
summary: Pages the Studio links to that do not exist yet - the ideas and pages it is asking for.
slug: ghost-pages
permalink: /ghost-pages
type: section
status: active
tags:
  - studio
---

<nav class="studio-listnav"><a href="/entries-list-alphabetical">Entries A to Z</a><a href="/entries-list-most-recent">Recent changes</a><a href="/tags-list-alphabetical">Tags A to Z</a><a href="/types-list">Types A to Z</a><a href="/ghost-pages" class="is-current">Ghost pages</a></nav>

Pages that other pages link to, but that have not been written yet. 3 wanted and 5 ideas, most-linked first. Generated from the vault on 24 September 2026.

## Wanted

Pages the Studio clearly means to have.

- **Book I  -  Constitution for Humanity** (6) - linked from [[005 Principles|Principles]], [[006 Models|Models]], [[007 Frameworks|Frameworks]], [[008 Systems|Systems]], [[009 Patterns|Patterns]], [[010 Composition|Composition]]
- **Canon** (2) - linked from [[Home]], [[Stewardship]]
- **Architecture Should Emerge** (1) - linked from [[Home]]

## Ideas

Single terms linked in passing. Each could grow into a glossary entry.

- **communities** (1) - linked from [[Customer Effective Demand Network]]
- **customers** (1) - linked from [[Customer Effective Demand Network]]
- **decisions** (1) - linked from [[Customer Effective Demand Network]]
- **partners** (1) - linked from [[Customer Effective Demand Network]]
- **platforms** (1) - linked from [[Customer Effective Demand Network]]
